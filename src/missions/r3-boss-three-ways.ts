 ...
 *
 * Comps range 7.4x-9.5x runs from Ironvale Components (7.4x, the lowest
 * genuine industrial peer multiple) to Palisade Doors & Docks (9.5x, the
 * highest) — trading multiples, no control premium.
 * Precedents range 7.8x-9.2x spans the full PRECEDENTS list (Marrow
 * Fabrication/Palisade 7.8x low, Trestle Foods/Copperline 9.2x high) —
 * whole-company acquisitions, so a control premium is baked in.
 ...
 *   comps high:      (9.5 x 96,000 - 220,000) / 40,000 = 17.30
 *   precedents low:  (7.8 x 96,000 - 220,000) / 40,000 = 13.22
 *   precedents high: (9.2 x 96,000 - 220,000) / 40,000 = 16.58
...
const mission: Mission = {
  ...
  lesson: {
    ...
    body:
      "A football field chart lines up several valuation ranges for the same company. Each multiplies EBITDA (operating profit before interest, tax, depreciation, and amortization) by a multiple for enterprise value (EV, the price for the whole business), subtracts net debt (debt minus cash) for equity value, then divides by shares outstanding for a price per share. Trading comps use multiples paid for similar public companies today. Precedent transactions use multiples from past whole-company acquisitions, so they carry a control premium: the extra a buyer pays to take over and run the company. The 52-week range is just where the stock has traded. Selling the whole company? Defend the precedent range — it is the only one priced for control.",
    visual: {
      kind: 'bullets',
      items: [
        `Comps 7.4x–9.5x EBITDA (96,000) → EV 710,400–912,000 − net debt 220,000 → $${COMPS_LOW.toFixed(2)}–$${COMPS_HIGH.toFixed(2)}/share`,
        `Precedents 7.8x–9.2x EBITDA → EV 748,800–883,200 − net debt → $${PRECEDENTS_LOW.toFixed(2)}–$${PRECEDENTS_HIGH.toFixed(2)}/share`,
  ...
  task: {
    ...
    rows: [
      {
        id: 'comps',
        label: 'Trading comps 7.4x–9.5x',
        lowAnswer: COMPS_LOW,
        highAnswer: COMPS_HIGH,
        tolerance: 0.3,
        role: 'equity',
        hint: 'Ironvale Components (7.4x) to Palisade Doors & Docks (9.5x): EV = multiple x EBITDA 96,000; minus net debt 220,000; over 40,000k shares',
      },
      {
        id: 'precedents',
        label: 'Precedents 7.8x–9.2x',
        lowAnswer: PRECEDENTS_LOW,
        highAnswer: PRECEDENTS_HIGH,
        tolerance: 0.3,
        role: 'debt',
        hint: 'Past whole-company acquisitions: EV = multiple x EBITDA 96,000; minus net debt 220,000; over 40,000k shares',
      },
      {
        id: 'market',
    ...
    question: {
      ...
      explanation:
        'Precedents price whole-company acquisitions — the same kind of deal your client is doing, where a buyer pays for control, not just a minority stake. That control premium (the extra a buyer pays over the trading price to take over and run the target) is baked into the 7.8x–9.2x range, which is why it is the range you would defend to a client selling the whole company. Trading comps price minority stakes with no control premium, and the 52-week range just reflects where the stock happened to trade, not what a buyer would pay for control.',
      weight: 0.25,
    },
  },
  grade(answer) {
