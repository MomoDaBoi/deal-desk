...
const LD_PE_LINE = `Ledgerly's P/E is the same equity-value-over-net-income divide: ${money(LD_MARKET_CAP)} ÷ ${money(LD_NET_INCOME)} = ${LD_PE}x. That number looks absurd because it is: a ${LD_GROSS_MARGIN_PCT}% gross-margin business whose net income line is thin — interest of ${money(LEDGERLY.income.interest)} and tax of ${money(LEDGERLY.income.taxes)} take an ${money(LEDGERLY.income.ebit)} EBIT (itself already after ${money(LEDGERLY.income.da ?? 0)} of D&A) down to just ${money(LD_NET_INCOME)} of net income — so the P/E says almost nothing. This is the case where you reach for EV/EBITDA or EV/Revenue instead.`
